...
export function setupGUI(parentContext) {
  ...
  styleEl.textContent = `
    *,*::before,*::after{box-sizing:border-box}
    ...
    /* ── Activity buttons ── */
    .rcp-act-btn-grid { display: flex; flex-direction: column; gap: 6px; padding: 4px 0; }
    .rcp-activity-btn {
    ...
    .rcp-activity-btn:hover {
      background: linear-gradient(135deg, rgba(99,102,241,0.25), rgba(139,92,246,0.18));
      border-color: rgba(99,102,241,0.6);
    }
    .rcp-activity-btn:active { transform: scale(0.97); }
    .rcp-activity-btn .rcp-act-icon { font-size: 16px; line-height: 1; }
    .rcp-activity-btn .rcp-act-label { flex: 1; }
    .rcp-activity-btn .rcp-act-tag {
      font-size: 9px; padding: 2px 6px;
      background: rgba(99,102,241,0.2); border-radius: 10px;
      color: #818cf8; font-weight: 700; letter-spacing: 0.03em;
    }
    .rcp-activity-btn.active {
      background: linear-gradient(135deg, rgba(99,102,241,0.35), rgba(139,92,246,0.28));
      border-color: rgba(99,102,241,0.8);
      color: #e0d7ff;
    }
    .rcp-activity-btn.active .rcp-act-tag { background: rgba(99,102,241,0.45); color: #c7d2fe; }
  `;
  ...
  // ── Activities card ───────────────────────────────────────────────────
  const { card: cardActivities, body: bodyActivities } = makeCard('Activities', '1 act', 'rcp-badge-purple', true);
  const actBtnGrid = document.createElement('div');
  actBtnGrid.className = 'rcp-act-btn-grid';
  bodyActivities.appendChild(actBtnGrid);
  scroll.appendChild(cardActivities);

  // Helper: find actuator index by name
  const findActuatorIndex = (model, name) => {
    const td = new TextDecoder('utf-8');
    for (let i = 0; i < model.nu; i++) {
      const start = model.name_actuatoradr[i];
      const slice = model.names.subarray(start);
      const end   = slice.indexOf(0);
      const n     = td.decode(end >= 0 ? slice.subarray(0, end) : slice);
      if (n === name) return i;
    }
    return -1;
  };

  // Helper: smoothly drive a set of actuators to target ctrl values over ~600 ms
  const _activeAnimations = {};
  const animateActuators = (id, targets, durationMs = 600) => {
    // targets: [ { name, value }, ... ]
    if (_activeAnimations[id]) cancelAnimationFrame(_activeAnimations[id].raf);
    const model = parentContext.model;
    const data  = parentContext.data;
    const start = performance.now();

    // Resolve actuator indices and capture start values
    const resolved = targets.map(t => {
      const idx = findActuatorIndex(model, t.name);
      return { idx, from: idx >= 0 ? data.ctrl[idx] : 0, to: t.value };
    }).filter(r => r.idx >= 0);
    ...
    const step = (now) => {
      ...
      for (const r of resolved) {
        data.ctrl[r.idx] = r.from + (r.to - r.from) * ease;
      }
      if (t < 1.0) {
        _activeAnimations[id] = { raf: requestAnimationFrame(step) };
      } else {
  ...
  // ── Left Fist activity ─────────────────────────────────────────────
  // Joint targets derived from ctrlrange limits for a closed fist:
  //   thumb_0:   curl inward  →  0.8  rad  (range -1.05 to 1.05)
  //   thumb_1:   flex         →  0.8  rad  (range -0.72 to 1.05)
  //   thumb_2:   flex         →  1.2  rad  (range  0    to 1.75)
  //   middle_0:  max curl     → -1.5708 rad (range -1.57 to 0)
  ...
  const LEFT_FIST_TARGETS = [
    ...
    { name: 'left_hand_middle_0', value: -1.5708 },
    { name: 'left_hand_middle_1', value: -1.7453 },
  ...
  const LEFT_OPEN_TARGETS = [
    ...
    { name: 'left_hand_middle_0', value: 0 },
  ...
  fistBtn.className = 'rcp-activity-btn';
  fistBtn.innerHTML = `
    <span class="rcp-act-icon">✊</span>
    <span class="rcp-act-label">Left Hand Fist</span>
    <span class="rcp-act-tag">LEFT HAND</span>
  `;
  fistBtn.title = 'Close the left hand into a fist (click again to open)';
  fistBtn.addEventListener('click', () => {
    _leftFistActive = !_leftFistActive;
    if (_leftFistActive) {
      fistBtn.classList.add('active');
